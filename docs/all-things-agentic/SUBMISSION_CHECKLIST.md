...
| Repository URL | <https://github.com/a63909/StudioGridAI> | READY |
| Repository visibility/access | Public | READY |
| README spin-up instructions | Root `README.md` | READY |
| Architecture source | `architecture.mmd` | READY |
| Architecture image | `assets/studiogrid-architecture.png` | READY |
| Demo video | English or accurate English subtitles; public YouTube/Vimeo; maximum 4:00 | **USER ACTION: record, review, upload** |
| Application in action | Continuous one-command Coverage result, then Schedule PENDING/authority boundary | SCRIPT/SHOTLIST READY |
...
| Backend tests | PASS — current committed backend suite |
| Frontend tests | PASS — current committed frontend suite, including Production Command UX |
| Lint | PASS |
| Typecheck | PASS |
...
2. Manually capture sanitized Agent Engine and Firestore stills if they will be used.
3. Record the scripted video, verify runtime at 1× is at most 4:00, and ensure the continuous Coverage action, Schedule boundary, and Google Cloud proof are readable.
4. Upload the video publicly to YouTube or Vimeo.
5. Optionally publish the article and social post, then add their URLs.
6. Paste the English copy and links into Devpost, select The Taskmaster, preview everything, and submit before the deadline.

The repository is already public. No video upload, publication, or Devpost submission is authorized by this checklist.
